...
%% Device
% EPC
Data = readtable("Switch Functions\OneDevice_Sec.xlsx");
...
% Conduction Loss
duty_cond = 0.5;
P_cond = duty_cond * (I_d^2) * R_ds_max;

% Turn-off Loss
P_off  = ((I_d^2) * ((t_f*1e-9)^2) / (24*2*Coss*1e-12)) / (1/f_sw_max);

% Gate Loss
P_gate = V_g * Q_g * 1e-9 * f_sw_max;

P_total = P_off + P_cond + P_gate;

T_j = (P_total) * (Rth_jc   + Rth_inter + Rth_board_min + Rth_pw) + T_water;
