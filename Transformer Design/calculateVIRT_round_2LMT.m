function result = calculateVIRT_round_2LMT(b, w, params)
...
switch params.gap_loc
    case 'center'
        % Gap only in center post: full reluctance seen by Lm
        result.lg = params.u0 * params.np^2 * Ac / (params.nt * params.Lu);
    case 'all'
        % Gap in all legs: effective reluctance is halved
        result.lg = params.u0 * params.np^2 * Ac / (2 * params.nt * params.Lu);
    otherwise
