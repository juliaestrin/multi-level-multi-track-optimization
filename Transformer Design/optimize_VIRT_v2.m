...
% OPTIMIZE_VIRT_v2 Minimize P_total by optimizing a, b, w using fmincon
    ...
    if strcmp(params.centerpost_shape, 'round')
        results = calculate_VIRT_circularPost_design_v2(x_opt(2), x_opt(3), params);
    else 
        results = calculate_VIRT_design_v2(x_opt(1), x_opt(2), x_opt(3), params);
    end 

    fprintf('\nOptimal: a=%.2fmm, b=%.2fmm, w=%.2fmm, P_total=%.4fW\n', ...
        x_opt(1)*1e3, x_opt(2)*1e3, x_opt(3)*1e3, P_opt);
end
