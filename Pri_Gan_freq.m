...
for k = 1:numel(f_list)
    ...
    outs{k} = analyzePriGaNSwitches( ...
        Power, f_list(k), I_r.I_r, mode, max_para, selected_para, compare_list, loss_limit);
end
...
%% ===================== Defaults =====================
if nargin < 9 || isempty(dataFile)
    dataFile = 'GaN Data.xlsx';
end

if nargin < 8 || isempty(max_total_loss)
    max_total_loss = inf;   % no loss limit unless user sets it
end
...
%% ===================== Current rating =====================
I_r_pk = I_r*sqrt(2);
I_rating = ceil(I_r_pk/5)*5; % Add some offset? 
...
for ii = 1:n_sw
    ...
    Rth_jc = GaNData{ii,4};
    R_ds_max = GaNData{ii,10};
    E_loss = GaNData{ii,6}; % [uJ]
    Coss = GaNData{ii,7}; % [pF]
    t_f = GaNData{ii,13}; % [ns]
    Q_g = GaNData{ii,14}; % [nC]
    V_g = GaNData{ii,15};
    T_j_max = GaNData{ii,11};
    ...
    for k = 1:nJ
        ...
        P_off(ii,k) = (jj)*((I_d^2)*((t_f*(1e-9))^2)/(24*2*Coss*(1e-12)))/(1/(f_sw_max/2));

        duty_cond = 0.25;
        P_cond(ii,k) = duty_cond * jj * (I_d^2) * R_ds_max;
        P_gate(ii,k) = jj * V_g * Q_g * 1e-9 * (f_sw_max/2);

        P_total(ii,k) = P_off(ii,k) + P_cond(ii,k) + P_gate(ii,k);
        ...
        thermal_check = T_j(ii,k) < (T_j_max - 30);
        loss_check = P_total(ii,k) <= max_total_loss;
...
best = pickBestDevices(Area, P_total_plot, Loss_Area_plot, jj_set, compare_list, GaNData);
