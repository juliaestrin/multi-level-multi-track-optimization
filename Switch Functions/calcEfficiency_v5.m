...
%         jj75_pri = value
    ...
    %v5 Update: keep full table, then compute final overall Pareto table

    % Full candidate table: all generated combinations
    effOut.table = T;
    ...
    if numel(pts) > 1
        ...
        isPareto = true(size(area_vec));
    end

    paretoIdx = find(isPareto);

    % Sort Pareto front by increasing area
    ...
    % Since T was sorted, use point_id to find corresponding Pareto rows
    [tf, loc] = ismember(paretoIdx, T.point_id);
    effOut.overallParetoTable = T(loc(tf), :);

    % Sort displayed Pareto table by area
    effOut.overallParetoTable = sortrows( ...
        effOut.overallParetoTable, ...
        {'Area [mm2]','Total Loss [W]'}, ...
        {'ascend','ascend'} ...
    );

    if showTable
        fprintf('\nFinal overall Pareto table:\n');
        disp(effOut.overallParetoTable);

        fprintf('\nFull candidate table is stored in effOut.table.\n');

        if doPlot
            fTbl = figure(figNum + 2); clf(fTbl);
            fTbl.Name = sprintf('Final Overall Pareto Table (%s pareto)', paretoSide);

            uitable(fTbl, ...
                'Data', effOut.overallParetoTable{:,:}, ...
                'ColumnName', effOut.overallParetoTable.Properties.VariableNames, ...
                'Units','normalized', ...
                'Position',[0 0 1 1]);
        end
    end
