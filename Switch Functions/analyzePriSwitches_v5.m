...
%% ===================== Defaults =====================
if nargin < 12, sicFile = 'SiC Data.xlsx'; end
if nargin < 11,  ganFile = 'GaN Data.xlsx'; end

if nargin < 10 || isempty(max_total_loss), max_total_loss = inf; end
if nargin < 9 || isempty(compare_list),  compare_list = [];  end
...
idx.GaN.Qg     = 14;
idx.GaN.Vg     = 15;
idx.GaN.Pin_Area     = 25; % Does not exist
...
% Thermal Resistancec Calculation
Rth_board_vec = nan(n_sw,1);
Rth_via_vec   = nan(n_sw,1);
Rth_fr4_vec   = nan(n_sw,1);
...
for ii_global = 1:n_sw
    ...
    for k = 1:nJ
        if topology == "Multilevel Multitrack"
            ...
            % ----- Area -----
            Area(ii_global,k) = (n25*jj25 + n75*jj75) * (L_min * W_min + Pin_Area);
            ...
            Area(ii_global,k) = pri_sw_count * jj * (L_min * W_min + Pin_Area);
            I_d = I_rating / jj;
